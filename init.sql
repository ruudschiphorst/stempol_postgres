...
CREATE TABLE IF NOT EXISTS multimedia (id SERIAL PRIMARY KEY, multimediaID UUID DEFAULT public.uuid_generate_v4() NOT NULL, title varchar(40) NOT NULL CHECK (title <> ''), filepath varchar(250), filetype varchar(40), noteID UUID NOT NULL, noteVersion integer NOT NULL, FOREIGN KEY (noteID, noteVersion) REFERENCES notes (noteID, version), UNIQUE(multimediaID));
